...
local Y = { garden_items_use_player_walk = false, auto_expand_garden = false, auto_expand_garden_max_slot = 2, player_speed_enabled = false, player_speed_value = 80, web_api_key =
...
"Random", seed_place_saved_position = {}, seed_place_max_garden_plants = 800, seed_place_overrides = {}, seed_place_wall_mode = false, seed_place_stack_mode = false, seed_place_stack_mode_underground = false, auto_sell_sellallinventory = false, sell_use_filters = false, sell_fruit_list = {}, sell_min_weight = 0, sell_max_weight = 89, sell_mutation_whitelist = {}, sell_mutation_blacklist = {}, sell_variant_whitelist = {}, sell_variant_blacklist = {}, turbo_sell = false, hide_log_ui = false, collection_teleport = true, char_farm_middle = false, auto_shovel_fruits = false, shovel_fruit_types = {}, shovel_mutation_whitelist = {}, shovel_mutation_blacklist = {}, shovel_min_weight = 0, shovel_max_weight = 99, shovel_variants = { Normal = true, Gold = true, Rainbow = true }, auto_shovel_plants = false, shovel_plant_types = {}, shovel_plant_min_height = 0, shovel_plant_max_height = 200, shovel_plant_variants = {}, shovel_growing_plants = false, shovel_plants_keep = 10, trowel_plant_types = {}, trowel_use_fixed_spot = true, trowel_saved_position = {}, pet_return_farm = false, pet_return_farm_timer = 60, mail_manual_batch_together = false, mail_auto_batch_together = false, mail_auto_send_enabled = false, mail_auto_accept = false, mail_include_comment = false, mail_next_send_at = 0, mail_manual_order = {}, mail_auto_rules = {}, mail_receipts = {}, mail_ignore_batch_limit = false, auto_use_daily_deal = true, step_teleport_speed = 60 }
...
d.GameApi = { Url = "", Busy = false, Started = false, GetInterval = function() return 17 end, GetRequestFunction = function()
    ...
    true, sheckles = tostring(d.Money.GetSheckles() or 0), pets_data = G }
end, Send = function()
...
d.FruitFilters = { RoundWeight = function(G)
    ...
    local G = {}
    for V, y in ipairs(d.FruitFilters.GetAllFruits()) do
        local Z = tostring(y.name or "")
        if Z == "" then continue end
        if not G[Z] then G[Z] = { total = 0, ready = 0 } end
        G[Z].total += 1
        if d.FruitFilters.IsFruitReady(y.ob) then G[Z].ready += 1 end
    end
    return G
...
d.FarmDetails = { Label = nil, Busy = false, Started = false, RefreshDelay = 10, GetWeight = function(G)
...
end, GetFruitObjects = function(G, V)
    local y = {}
    ...
    for G, V in ipairs(q) do table.insert(g,
    ...
    for G, V in ipairs(a) do
        ...
        for G, V in ipairs(Z) do table.insert(E,
                string.format(
    ...
    if #H > 0 then
        ...
        for G, V in ipairs(H) do table.insert(E,
...
d.FruitCollect = { FruitBucket = {}, FruitBucketIndex = 1, IsMaxFruitInventory = function()
    ...
    if d.FruitCollect.FruitBucketIndex > #d.FruitCollect.FruitBucket then
        d.FruitCollect.FruitBucket = d.FruitFilters.GetReadyFruits(nil, true) or {}
        d.FruitCollect.FruitBucketIndex = 1
    end
    local G = 0
    ...
    for V = 1, 500, 1 do
        local y = d.FruitCollect.FruitBucket[d.FruitCollect.FruitBucketIndex]
        if not y then
            d.FruitCollect.ResetBucket()
            break
        end
        if G >= 500 then break end
        d.FruitCollect.FruitBucketIndex += 1
        if not Y.auto_collect_fruit_enabled then return end
        if d.FruitCollect.IsMaxFruitInventory() then return end
        if not d.FruitFilters.PassesCollectorFilters(y) then continue end
        local Z = y.plantId
        local j = y.fruitId
        if d.FruitCollect.CollectFruit(Z, j) then
            G += 1
            task.wait()
        end
    end
