...
local Y = {
    high_mode = false,
    garden_items_use_player_walk = false,
...
d.RuntimeStats = {
    ...
    GetNetworkRuntimeStats = function()
        ...
        return {
            send_kbps = d.RuntimeStats.GetStatsItemRuntimeStats(V, { "Data Send Kbps", "Send Kbps" }),
            recv_kbps = d
    ...
    BuildPayloadRuntimeStats = function()
        ...
        return {
            ...
            min_fps = d
                .RuntimeStats.MinFpsRuntimeStats ~= 999 and d.RuntimeStats.MinFpsRuntimeStats or 0,
            max_frame_ms = d.RuntimeStats
                .MaxFrameMsRuntimeStats,
            ram_mb = d.RuntimeStats.GetMemoryRuntimeStats(),
            cpu_ms = d.RuntimeStats.GetCpuRuntimeStats(),
            ping_ms =
                d.RuntimeStats.GetPingRuntimeStats(),
            send_kbps = G.send_kbps,
            recv_kbps = G.recv_kbps,
            uptime_s = math.floor(os
                .clock() - d.RuntimeStats.StartedAtRuntimeStats),
            ...
            active_phase = tostring(y.Workspace:GetAttribute(
                "ActivePhase") or "")
        }
    end,
...
d.GameApi = {
    ...
    GetSeedsGameApi = function()
        ...
        for V, y in pairs(J.SeedShopDataList) do
            ...
            if type(Z) == "table" then
                table.insert(G,
                    {
                        name = tostring(Z.name or V),
                        rarity = tostring(Z.rarity or "Unknown"),
                        price = tonumber(Z.price) or 0,
                        stock =
                            d.GameApi.GetSeedStockGameApi(V),
                        single = Z.single == true,
                        icon_id = tonumber(Z.icon_id) or 0
                    })
            end
        end
        table.sort(G,
    ...
    GetGearGameApi = function()
        ...
        for V, y in ipairs(J.AllGearShopTable) do
            if type(y) == "table" then
                ...
                if V ~= "" then
                    table.insert(G,
                        {
                            name = V,
                            type = tostring(y.type or "Unknown"),
                            rarity = tostring(y.rarity or "Unknown"),
                            price =
                                tonumber(y.price) or 0,
                            stock = d.GameApi.GetGearStockGameApi(V),
                            icon_id = tonumber(y.icon_id) or 0
                        })
                end
            end
        end
...
d.PetEquipTriggers = {
    ...
    ActiveRuleId = tostring(Y
        .pet_equip_active_rule_id or ""),
    ActiveSignature = "",
    FinishedEventKeys = {},
    RulePriority = { Manual = 100, ["Seed Pack"] = 90, ["Dropped Seed"] = 85, ["Time Cycle Weather"] = 80, Weather = 70, ["Time Cycle Phase"] = 60, Default = 10 },
    TriggerOptions = { Manual = { "Manual" }, Default = { "Idle" }, ["Time Cycle"] = { "Goldmoon", "Rainbow Moon", "Bloodmoon", "Moon", "Night", "Day", "Sunset" }, Weather = { "Rain", "Lightning", "Rainbow", "Snowfall", "Starfall" }, ["Seed Pack"] = { "Any Seed Pack", "Gold Seed", "Rainbow Seed" }, ["Dropped Seed"] = { "Own Dropped Seed" } },
    SetStatusPetEquipTriggers = function(
        G, V)
        G = tostring(G or "")
        if G == "" then
            J.PetEquipTriggerStatusText = ""
    ...
    IsSelectionEmptyPetEquipTriggers = function(
        G)
                    ...
                    table.insert(V,
                        {
                            ...
                            size =
                                d.PetEquipTriggers.GetPetSizePetEquipTriggers(Z),
                            variant = d.PetEquipTriggers
                                .GetPetVariantPetEquipTriggers(Z),
    ...
    GetPetToolInfoPetEquipTriggers = function(G, V)
        ...
        return {
            ...
            size =
                d.PetEquipTriggers.GetPetSizePetEquipTriggers(c),
            variant = d.PetEquipTriggers.GetPetVariantPetEquipTriggers(c),
            equipped =
                type(V) == "table" and V[Z] == true or false,
            tool = G,
            data = c
        }
    end,
        ...
        for Z, j in pairs(j) do
            if type(j) == "table" then
                ...
                if i ~= "" and not ((type(V) == "table" and V[i])) then
                    local V = d.PetEquipTriggers.ResolvePetNamePetEquipTriggers(j.Name or j.Pet or j.Species or j
                        .PetName or
                        "")
                    if V ~= "" then
                        table.insert(y,
                            {
                                ...
                                id = i,
                                name = V,
                                displayName = d.PetEquipTriggers
                                    .GetPetDisplayNamePetEquipTriggers(V),
                                rarity = d.PetEquipTriggers.GetPetRarityPetEquipTriggers(
                                    V, j),
                                size = d.PetEquipTriggers.GetPetSizePetEquipTriggers(j),
                                variant = d.PetEquipTriggers
                                    .GetPetVariantPetEquipTriggers(j),
    ...
    GetExactPetDropdownPetEquipTriggers = function()
        ...
        for V, y in ipairs(d.PetEquipTriggers.GetInventoryPetsPetEquipTriggers()) do
            ...
            local j = string.format("%s | %s | %s | %s%s", tostring(y.displayName or y.name),
    ...
    CleanRuleNamePetEquipTriggers = function(
        G)
    ...
    AddRulePetEquipTriggers = function(G, V, y, Z, j)
        ...
        local T = d.PetEquipTriggers.NewRuleIdPetEquipTriggers(); (d.PetEquipTriggers.GetRulesTablePetEquipTriggers())[T] = {
            id =
                T,
    ...
    RemoveRulePetEquipTriggers = function(G)
        ...
        if d.PetEquipTriggers.ActiveRuleId == G or tostring(Y.pet_equip_active_rule_id or "") == G then
            d.PetEquipTriggers
                .RestorePreviousPetEquipTriggers("Deleted active loadout")
        end
        u.Save.SaveDataSync()
    ...
    GetWeatherValuesPetEquipTriggers = function()
        return y.ReplicatedStorage and
            y.ReplicatedStorage:FindFirstChild("WeatherValues") or nil
    end,
    NamesMatchPetEquipTriggers = function(G, V)
        return d
            .PetEquipTriggers.NormaliseTextPetEquipTriggers(G) == d.PetEquipTriggers.NormaliseTextPetEquipTriggers(V)
    end,
    IsTimeCycleWeatherPetEquipTriggers = function(
        G)
        local V = d.PetEquipTriggers.NormaliseTextPetEquipTriggers(G)
    ...
    GetRulePriorityPetEquipTriggers = function(G)
        ...
        if V == "Time Cycle" then
            if d.PetEquipTriggers.IsTimeCycleWeatherPetEquipTriggers(G.triggerName) then
                return d.PetEquipTriggers
                    .RulePriority["Time Cycle Weather"] or 80
            end
    ...
    GetTimeCycleActivePetEquipTriggers = function(G)
        ...
        if d.PetEquipTriggers.IsTimeCycleWeatherPetEquipTriggers(G) then
            return d.PetEquipTriggers
                .NamesMatchPetEquipTriggers(y.Workspace:GetAttribute("ActiveWeather"), G)
        end
        if d.PetEquipTriggers.IsTimeCyclePhasePetEquipTriggers(G) then
            return d.PetEquipTriggers.NamesMatchPetEquipTriggers(
                y.Workspace:GetAttribute("ActivePhase"), G)
        end
        return false
    end,
    ...
    GetRuleConditionPetEquipTriggers = function(G)
        ...
        elseif V == "Weather" then
            Z =
                d.PetEquipTriggers.GetWeatherActivePetEquipTriggers(y)
        elseif V == "Time Cycle" then
            Z = d.PetEquipTriggers
                .GetTimeCycleActivePetEquipTriggers(y)
        elseif V == "Seed Pack" then
            Z = d.PetEquipTriggers
                .GetSeedPackActivePetEquipTriggers(y)
        elseif V == "Dropped Seed" then
            Z = d.PetEquipTriggers
                .GetDroppedSeedActivePetEquipTriggers()
        end
    ...
    BuildSnapshotPetEquipTriggers = function(G)
        ...
        for G, y in ipairs(type(G) == "table" and G or {}) do
            ...
            if Z ~= "" then
                table.insert(V,
                    { id = Z, pet = tostring(y.name or ""), size = tostring(y.size or ""), variant = tostring(y.variant or
                    "") })
            end
        end
        return V
            ...
            if y then
                ...
                for G, y in pairs(G) do
                    ...
                    if Z ~= "" and (tonumber(y) and tonumber(y) > 0) then
                        table.insert(V,
                            { pet = Z, amount = math.floor(tonumber(y)), size = "", variant = "" })
                    end
                end
            end
        end
    ...
    ApplyRulePetEquipTriggers = function(G, V)
        ...
        local Z, j = pcall(function()
            ...
            if g ~= "restore" and (tostring(G.triggerType or "") ~= "Default" and (Y.pet_equip_restore_previous and d.PetEquipTriggers.ActiveRuleId == "")) then
                Y.pet_equip_restore_snapshot =
                    d.PetEquipTriggers.BuildSnapshotPetEquipTriggers(E)
            end
            local a = {}
    ...
    ProcessPetEquipTriggers = function()
        if not Y.pet_equip_enabled then
            if d.PetEquipTriggers.ActiveRuleId ~= "" or tostring(Y.pet_equip_active_rule_id or "") ~= "" then
                d
                    .PetEquipTriggers.RestorePreviousPetEquipTriggers("System disabled")
            end
        ...
        d.PetEquipTriggers.SetStatusPetEquipTriggers(
            y and "Manual ready - press Run Manual Now" or "Waiting for trigger",
            "#CFCFCF")
        return false
    end,
    HookEventsPetEquipTriggers = function()
        ...
        if G then
            for V, y in ipairs(d.PetEquipTriggers.TriggerOptions.Weather) do
                (G:GetAttributeChangedSignal(y .. "_Playing"))
                    :Connect(function()
        ...
        end; (y.Workspace:GetAttributeChangedSignal("ActivePhase")):Connect(function()
            d.PetEquipTriggers.AddPetEquipLog("Phase changed: " ..
...
d.PlayerSpeed = {
    ...
    ApplyPlayerSpeed = function()
        ...
        d.PlayerSpeed.SetStatusPlayerSpeed(
            string.format("Minimum %d \226\128\162 Current %d", V, math.floor(G.WalkSpeed + .5)),
            "#66CCFF")
        return true
    end,
...
d.SellManager = {
    ...
    AreSellFiltersActive = function() return Y.sell_use_filters == true end,
    ...
    GetSellFruitTypeDropdownWithBackpackCounts = function()
        ...
        for y, Z in ipairs(J.AllSeedsDataTable) do
            ...
            table.insert(G,
                {
                    Text = string.format(
                        "<font color=\"#FFFFFF\">%s</font> <font color=\"%s\">%s</font> <font color=\"#7CFC00\">x%d</font>",
                        j, c, i,
                        tonumber(V[j]) or 0),
                    Value = j
                })
    ...
    SellFilteredBackpack = function(G)
        ...
        if y > 0 then
            d.SellManager.SetStatus(
                string.format("Filtered sold %d fruit%s | +$%s", y, y == 1 and "" or "s", c.formatShecklesNumber(J)),
                "#66FF99")
...
d.PetSeller = {
    Busy = false,
    NextRunAt = 0,
    RarityOrder = { "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Super", "Secret" },
    SetStatus = function(
        G, V)
        if type(G) ~= "string" or G == "" then
            J.PetSellerStatusText = ""
    ...
    GetDelay = function() return math.clamp(tonumber(Y.pet_sell_delay) or .25, .15, 10) end,
    GetKeepAmount = function()
        return
            math.max(math.floor(tonumber(Y.pet_sell_keep_amount) or 1), 0)
    end,
    GetMaxPerCycle = function()
        return math.clamp(
            math.floor(tonumber(Y.pet_sell_max_per_cycle) or 3), 1, 100)
    end,
    ...
    GetBucketKey = function(G)
        ...
        return table.concat({ tostring(G.name or ""), tostring(G.size or "Normal"), tostring(G.variant or "Normal") },
            "\031")
    end,
    ...
    RunPetSellerCycle = function(G)
        ...
        if Z > 0 then
            d.PetSeller.SetStatus(
                string.format("Sold %d pet%s | +$%s", Z, Z == 1 and "" or "s", c.formatShecklesNumber(i)),
                "#66FF99")
            ...
            table.insert(G,
                {
                    Text = string.format(
                        ...
                        y.displayName, Z, y.rarity, y.count, j, c.formatShecklesNumber(y.estimate)),
                    Value = y.name
                })
        end
...
d.FruitFiltersDataSync = {
    ...
    IsMine = function(G)
        return
            tonumber(G) == tonumber(J.player_userid)
    end,
    ...
    GetWeightFormat = function() return type(y.WeightFormat) == "table" and y.WeightFormat or nil end,
    FormatKg = function(
        G)
    ...
    GetCalculateOvertimeGrowth = function()
        return type(y.CalculateOvertimeGrowth) == "function" and
            y.CalculateOvertimeGrowth or nil
    end,
    GetOvertimeFlags = function()
        return type(y.OvertimeGrowthFlags) == "table" and
            y.OvertimeGrowthFlags or nil
    end,
    ...
    IsSingleHarvestPlant = function(G)
        ...
        if d.SeedData and type(d.SeedData.IsSingleHarvest) == "function" then return d.SeedData.IsSingleHarvest(G) ==
            true end
    ...
    SyncFromGarden = function()
        ...
        for V, y in pairs(V) do
            ...
            G.EnsurePlant(V, y)
            if type(y.Fruits) == "table" then
                for Z, j in pairs(y.Fruits) do
                    if type(j) == "table" then
                        Z = tostring(Z or "")
    ...
    BuildFruitResult = function(G, V, y)
        ...
        return {
            ...
            age = y and Z.N(G.CurrentAge, G.Age or 0) or
                Z.N(V.CurrentAge, 0),
            max_age = y and Z.N(G.MaxAge, 0) or Z.N(V.MaxAge, 0),
            grow_rate = y and Z.GetPlantGrowthRate(G) or
                Z.N(V.GrowthRate, 0),
            ready = true,
            sync = true,
...
d.FruitFilters = {
    ...
    SplitMutations = function(G) return d.FruitFilters.ParseMutationText(G) end,
    ...
    GetFruitWeight = function(G)
        ...
        local V, Z = pcall(function()
            if type(y.FruitVisualizerController.CalculateFruitWeight) == "function" then
                return y.FruitVisualizerController
                    :CalculateFruitWeight(G)
            end
            return nil
        end)
        if not V or type(Z) ~= "number" then
            V, Z = pcall(function()
                if type(y.FruitVisualizerController.CalculatePlantWeight) == "function" then
                    return y
                        .FruitVisualizerController:CalculatePlantWeight(G)
                end
                return nil
            end)
        end
            ...
            table.insert(G,
                {
                    Text = string.format(
                        ...
                        j, c, i, tonumber(J.ready) or 0, tonumber(J.total) or 0),
                    Value = j
                })
        end
        return G
...
d.ShovelFruits = {
    ...
    GetMutationNames = function()
        return
            d.FruitFilters.GetMutationNames()
    end,
    GetVariantNames = function() return d.FruitFilters.GetVariantNames() end,
    ...
    SplitMutations = function(G) return d.FruitFilters.SplitMutations(G) end,
    GetVariantFromMutations = function(G)
        return
            d.FruitFilters.GetVariantFromMutations(G)
    end,
    HasSelectedMutation = function(G, V)
        return d.FruitFilters
            .HasSelectedMutation(G, V)
    end,
    PassesMutationFilters = function(G)
        return d.FruitFilters.PassesMutationSelection(G,
            Y.shovel_mutation_whitelist, Y.shovel_mutation_blacklist)
    end,
    ...
    BuildText = function()
        ...
        for G, i in pairs(G.Plants) do
            ...
            if d.FarmDetails.IsSingleHarvestFarmDetails(c) then
                d.FarmDetails.AddFruitDataFarmDetails(i, nil, true, q, j, Z,
                    y)
            else
                for G, V in pairs(i.Fruits or {}) do
                    d.FarmDetails.AddFruitDataFarmDetails(i, V, false, q, j, Z,
                        y)
                end
            end
        end
        if j.totalPlants <= 0 then
            return
            "<font color=\'#AFAFAF\'>No farm data found yet. Wait for garden load or rejoin.</font>"
        end
        local i = {}
        local c = {}
        local T = {}
...
d.FruitCollect = {
    ...
    GetReadyFruitBucket = function()
        ...
        if J.GetCheckIfPro() then
            if G and (d.FruitFiltersDataSync and type(d.FruitFiltersDataSync.GetFruits) == "function") then
...
d.PetFinderPremium = {
    ...
    BuyPet = function(G)
        ...
        local T = { ref = V, data = G, countBefore = d.PetFinderPremium.CountOwnedForRule(G.name, Z), startedAt = os
...
d.GardenItems = {
    ...
    PetSeedCollectSystem = {
        ...
        Claim = function(G)
            ...
            local Z = d.Teleport.LockTeleport(y, Y.garden_items_use_player_walk and 35 or 2,
                Y.garden_items_use_player_walk ==
                true)
            if not Z then
...
d.GardenItems.EventSeedCollectSystem = {
    ...
    Claim = function(G)
        ...
        local i, c = pcall(function()
            if Y.garden_items_use_player_walk then
                if not d.Movement.WalkPathToTarget(G, 35, j, 10) then return false end
            else
...
d.PlantModelCleaner = {
    ...
    IsActivePlantModelCleaner = function()
        return
            Y.high_mode == true and J.GetCheckIfPro() == true
    end,
    SetStatusPlantModelCleaner = function(G, V)
        J.PlantModelCleanerStatusText =
            string.format(
    ...
    RemovePlantByIdPlantModelCleaner = function(G)
        ...
        for V, y in ipairs(V:GetChildren()) do
            if tostring(y:GetAttribute("PlantId") or y:GetAttribute("PlantID") or y:GetAttribute("Id") or y.Name or "") == G then
                return
                    d.PlantModelCleaner.DestroyObjectPlantModelCleaner(y)
            end
        end
        return false
    end,
    ApplyPlantModelCleaner = function()
        ...
        if d.PlantModelCleaner.IsActivePlantModelCleaner() then
            d.PlantModelCleaner.SetStatusPlantModelCleaner(
                "Removing live plant models", "#7CFC00")
        else
        ...
        G = y:AddToggle("premiumperformance-",
            {
                ...
                Default =
                    Y.high_mode,
                Tooltip =
                "Premium speed mode. Removes garden plant models for less lag and faster fruit collection. Turn off and rejoin to bring plants back.",
                DisabledTooptip =
                    J.GetProMessage(),
                Callback = function(G)
                    Y.high_mode = G
        ...
        V:AddToggle("garden_items_use_player_walk_ui",
            {
                ...
                Tooltip =
                "Walks to seeds first, then snaps when close.",
                Callback = function(G)
...
J.CollectUi = function()
    ...
    if V then
        ...
        V:AddButton({
            ...
            Tooltip =
            "Refreshes farm fruit counts in the list.",
            Func = function()
                G:SetValues(d.FruitFilters.GetFruitTypeDropdownWithFarmCounts())
                G:SetValue(Y.collect_fruit_list)
                d.FruitCollect.ResetBucket()
        ...
        V:AddToggle("enablefruitcollector",
            {
                ...
                Tooltip =
                "Collects fruits and teleports you to the farm.",
                Callback = function(G)
    ...
    if y then
        y:AddLabel({
            Text =
            "\226\132\185\239\184\143 Press Enter after changing KG values. Empty mutation or variant lists collect everything unless protected below.",
            DoesWrap = true
        })
        local G
        local V
        ...
        i = y:AddValueDropdown("collect_mutation_whitelist",
            {
                ...
                Multi = true,
                Text =
                "\226\156\133 Only Mutations",
                Tooltip =
                "Only collect fruits with selected mutations. Empty allows all mutations.",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        T = y:AddValueDropdown("collect_variant_whitelist",
            {
                ...
                Tooltip =
                "Only collect selected variants. Empty allows all variants.",
                Searchable = false,
                MaxVisibleDropdownItems = 5,
                Changed = function(
                    G)
        ...
        q = y:AddValueDropdown("collect_variant_blacklist",
            {
                ...
                Multi = true,
                Text =
                "\240\159\155\161\239\184\143 Protect Variants",
...
J.SellingUi = function()
        ...
        V:AddToggle("auto_use_daily_deal_ui",
            {
                ...
                Tooltip =
                "Saves the Daily Deal until your fruit backpack is full. Filtered selling protects fruits and uses one-by-one selling.",
                Callback = function(
                    G)
        ...
        V:AddToggle("sell_when_backpack_full_ui",
            {
                ...
                Tooltip =
                "Automatically sells fruits when your backpack is full.",
                Callback = function(G)
    ...
    if y then
        y:AddLabel({
            Text =
            "\226\132\185\239\184\143 Enable filters to sell matching fruits one by one. Press Enter after changing KG values. Protected mutations or variants will not be sold.",
            DoesWrap = true
        })
        local G
        G = y:AddValueDropdown("sell_fruit_filter_list",
            {
                ...
                Tooltip =
                "Only selected fruits will be sold when filters are enabled. Empty allows all fruit names.",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        y:AddButton({
            ...
            Tooltip =
            "Refreshes backpack fruit counts in the list.",
            Func = function()
                G:SetValues(d.SellManager.GetSellFruitTypeDropdownWithBackpackCounts())
                G:SetValue(Y.sell_fruit_list)
            end
        ...
        y:AddToggle("sell_filters_enabled_ui",
            {
                ...
                Tooltip =
                "When enabled, only fruits passing the filters are sold one by one.",
                Callback = function(G)
    ...
    if Z then
        Z:AddLabel({
            Text =
            "\240\159\155\161\239\184\143 Pet seller only sells selected pets. Equipped, unknown, protected, Rainbow and Big/Huge pets are protected by default.",
            DoesWrap = true
        })
        Z:AddToggle("pet_sell_preview_only_ui",
            {
                ...
                Tooltip =
                "Shows how many pets are safe to sell without selling them.",
                Callback = function(G)
        ...
        Z:AddToggle("auto_sell_pets_ui",
            {
                ...
                Tooltip =
                "Sells only selected pets that pass all protection filters.",
                Callback = function(G)
                    Y.auto_sell_pets = G
        ...
        Z:AddToggle("pet_sell_duplicate_only_ui",
            {
                ...
                Tooltip =
                "Keeps the chosen amount for each pet size and variant bucket.",
                Callback = function(G)
        ...
        Z:AddButton({
            ...
            Tooltip =
            "Refreshes all pet seller dropdowns.",
            Func = function()
        ...
        G = i:AddValueDropdown("pet_sell_selected_ui",
            {
                ...
                Tooltip =
                "Only selected pets can be sold. Empty sells nothing.",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        V = i:AddValueDropdown("pet_sell_protected_ui",
            {
                ...
                Tooltip =
                "Selected pet names will not be sold.",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        y = i:AddValueDropdown("pet_sell_protected_ids_ui",
            {
                ...
                Tooltip =
                "Selected exact pet IDs will not be sold.",
                Searchable = true,
                MaxVisibleDropdownItems = 10,
                Changed = function(
                    G)
        ...
        i:AddDropdown("pet_sell_max_rarity_ui",
            {
                ...
                Multi = false,
                Text =
                "\226\173\144 Max Rarity To Sell",
        ...
        i:AddToggle("pet_sell_protect_big_huge_ui",
            {
                ...
                Tooltip =
                "Big and Huge pets will not be sold.",
                Callback = function(G)
        ...
        Z = i:AddValueDropdown("pet_sell_size_whitelist_ui",
            {
                ...
                Tooltip =
                "Only selected sizes can be sold. Empty allows all sizes.",
                Searchable = false,
                MaxVisibleDropdownItems = 5,
                Changed = function(
                    G)
        ...
        j = i:AddValueDropdown("pet_sell_size_blacklist_ui",
            {
                ...
                Tooltip =
                "Selected sizes will not be sold.",
                Searchable = false,
                MaxVisibleDropdownItems = 5,
                Changed = function(G)
                    if type(G) ~= "table" then return end
        ...
        c = i:AddValueDropdown("pet_sell_variant_whitelist_ui",
            {
                ...
                Text = "\226\156\133 Only Variants",
                Tooltip =
                "Only selected variants can be sold. Empty allows all variants.",
                Searchable = false,
                MaxVisibleDropdownItems = 5,
                Changed = function(
                    G)
        ...
        T = i:AddValueDropdown("pet_sell_variant_blacklist_ui",
            {
                ...
                Tooltip =
                "Selected variants will not be sold.",
                Searchable = false,
                MaxVisibleDropdownItems = 5,
                Changed = function(G)
                    if type(G) ~= "table" then return end
...
J.TestGardenSync = {
    ...
    Colours = { Title = "#7CFC00", Plant = "#6EE7FF", Fruit = "#FFFFFF", Ready = "#7CFC00", Growing = "#FFD166", Removed = "#FF5C5C", Muted = "#AAAAAA", Gold = "#FFD700", Rainbow = "#FF66FF", Normal = "#DADADA", Kg = "#7CFC00", Id = "#888888", Bad = "#FF5C5C" },
    Now = function()
        return
            os.clock()
    end,
    ...
    C = function(G, V) return string.format("<font color=\"%s\">%s</font>", G or "#FFFFFF", J.TestGardenSync.Html(V)) end,
    Key = function(
        G, V)
    ...
    GetSharedModules = function()
        return y.SharedModules or
            (y.ReplicatedStorage and y.ReplicatedStorage:FindFirstChild("SharedModules"))
    end,
    GetWeightFormat = function()
        if J.TestGardenSync.WeightFormatCache ~= nil then
            return J.TestGardenSync.WeightFormatCache ~= false and
                J.TestGardenSync.WeightFormatCache or nil
        end
    ...
    GetOvertimeFlags = function()
        if J.TestGardenSync.OvertimeFlagsCache ~= nil then
            return J.TestGardenSync.OvertimeFlagsCache ~= false and
                J.TestGardenSync.OvertimeFlagsCache or nil
        end
    ...
    GetCalculateOvertimeGrowth = function()
        if J.TestGardenSync.CalculateOvertimeGrowthCache ~= nil then
            return J.TestGardenSync.CalculateOvertimeGrowthCache ~=
                false and J.TestGardenSync.CalculateOvertimeGrowthCache or nil
        end
    ...
    GetGrowRateData = function()
        if J.TestGardenSync.GrowRateDataCache ~= nil then
            return J.TestGardenSync.GrowRateDataCache ~= false and
                J.TestGardenSync.GrowRateDataCache or nil
        end
    ...
    Stop = function()
        ...
        if J.TestGardenSync.Label then
            J.TestGardenSync.Label:SetText(J.TestGardenSync.C(J.TestGardenSync.Colours.Bad,
                "Garden sync test stopped."))
        end
        return true
    end,
    Reset = function()
    ...
    BuildFruitResult = function(G, V, y)
        ...
        return {
            ...
            v = d,
            age = y and
                J.TestGardenSync.N(G.CurrentAge, G.Age or 0) or J.TestGardenSync.N(V.CurrentAge, 0),
            max_age = y and
                J.TestGardenSync.N(G.MaxAge, 0) or J.TestGardenSync.N(V.MaxAge, 0),
            grow_rate = y and
                J.TestGardenSync.GetPlantGrowthRate(G) or J.TestGardenSync.N(V.GrowthRate, 0),
            ready = true,
            sync = true,
            single_harvest =
                y == true,
    ...
    GetReadyFruits = function() return J.TestGardenSync.GetFruits() end,
            ...
            table.insert(G,
                {
                    Text = string.format(
                        ...
                        c and " <font color=\"#FFD166\">Single</font>" or ""),
                    Value = y.Id
                })
        end
        return G
        ...
        V:AddToggle("player_speed_enabled_ui",
            {
                ...
                Tooltip =
                "Keeps your movement speed at the selected minimum.",
                Callback = function(G)
...
J.PetEquipTriggersUi = function()
    ...
    if V then
        ...
        V:AddToggle("pet_equip_enabled_ui",
            {
                ...
                Tooltip =
                "Runs enabled loadouts when their trigger is active.",
                Callback = function(G)
        ...
        V:AddToggle("pet_equip_restore_previous_ui",
            {
                ...
                Tooltip =
                "Restores the pets equipped before the trigger started.",
                Callback = function(G)
        ...
        V:AddToggle("pet_equip_log_enabled_ui",
            {
                ...
                Tooltip =
                "Stores the latest trigger events in the log group.",
                Callback = function(G)
        ...
        V:AddButton({
            ...
            Func = function()
                d.PetEquipTriggers
                    .RestorePreviousPetEquipTriggers("Manual restore")
            end
        })
        V:AddDivider()
        V:AddToggle("pet_equip_protect_rainbow_ui",
            {
                ...
                Tooltip =
                "Rainbow pets are kept equipped when possible.",
                Callback = function(G)
        ...
        V:AddToggle("pet_equip_protect_big_huge_ui",
            {
                ...
                Tooltip =
                "Big and Huge pets are kept equipped when possible.",
                Callback = function(G)
        ...
        V:AddToggle("pet_equip_protect_super_secret_ui",
            {
                ...
                Tooltip =
                "Super and Secret pets are kept equipped when possible.",
                Callback = function(G)
        ...
        G = V:AddValueDropdown("pet_equip_protected_names_ui",
            {
                ...
                MaxVisibleDropdownItems = 10,
                Text =
                "\240\159\155\161\239\184\143 Protect Pet Types",
                Tooltip =
                "Selected pet types will not be unequipped by triggers.",
                Callback = function(G)
        ...
        y = V:AddValueDropdown("pet_equip_protected_ids_ui",
            {
                ...
                MaxVisibleDropdownItems = 10,
                Text =
                "\240\159\148\144 Protect Exact Pets",
        ...
        local function y(y)
            ...
            if not i then
                ...
                i.Toggle = Z:AddToggle("pet_equip_rule_toggle_" .. j,
                    {
                        ...
                        Tooltip =
                        "Enable or disable this saved loadout.",
                        Callback = function(G)
                ...
                i.RunButton = Z:AddButton({
                    Text = tostring(y.triggerType or "") == "Manual" and
                        "\226\150\182 Run Manual Now" or "\226\150\182 Auto Trigger Only",
                    Func = function()
                ...
                i.DeleteButton = Z:AddButton({
                    ...
                    Func = function()
                        d
                            .PetEquipTriggers.RemoveRulePetEquipTriggers(j)
                    end
                })
                i.Divider = Z:AddLabel({
                    Text =
                    "<font color=\'#444444\'>\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128\226\148\128</font>",
                    DoesWrap = false
                })
                G[j] = i
            end
    ...
    if y then
        ...
        y:AddInput("pet_equip_rule_name_input_ui",
            {
                ...
                Callback = function(V)
                    G.name = d
                        .PetEquipTriggers.CleanRuleNamePetEquipTriggers(V)
                end
            })
        local V
        local Z
        ...
        V = y:AddDropdown("pet_equip_trigger_type_ui",
            {
                Values = { "Manual", "Default", "Time Cycle", "Weather", "Seed Pack", "Dropped Seed" },
                Default = G
                    .triggerType,
                Multi = false,
                Text = "Trigger Type",
        ...
        y:AddDropdown("pet_equip_pet_size_ui",
            {
                Values = { "Any", "Normal", "Big", "Huge" },
                Default = "Any",
                Multi = false,
                Text = "Optional Size",
                Tooltip =
                "Optional pet size filter.",
                Callback = function(G)
                    G = tostring(G or "Any")
                    T = G == "Any" and "" or G
                end
        ...
        y:AddDropdown("pet_equip_pet_variant_ui",
            {
                Values = { "Any", "Normal", "Rainbow" },
                Default = "Any",
                Multi = false,
                Text = "Optional Variant",
                Tooltip =
                "Optional pet variant filter.",
                Callback = function(G)
                    G = tostring(G or "Any")
                    u = G == "Any" and "" or G
                end
...
task.spawn(function()
    while true do
        ...
        if Y.auto_sell_pets and (type(J.PetSellerStatusText) == "string" and J.PetSellerStatusText ~= "") then
            table
                .insert(G, J.PetSellerStatusText)
        end
