...
function Addcantsleep() if (getconnections or get_signal_cons) then for G, V in pairs(((getconnections or get_signal_cons))(y.LocalPlayer.Idled)) do if V.Disable then
...
if not Z or type(Z.CreateWindow) ~= "function" then
    ...
    Z = loadstring(game:HttpGet("https://raw.githubusercontent.com/Reversed1337/Testing-123/refs/heads/main/zeta_hub_ui.lua"))()
    if not Z then
...
local Y = { webhook_enabled = true, webhook_url = "", webhook_pet_buys = true, webhook_mail_manual = true, webhook_mail_auto = true, webhook_mail_claims = true, pet_finder_enabled = false, pet_finder_buy_list = {}, pet_finder_auto_hop = false, pet_finder_hop_minutes = 5, pet_finder_purchase_log = {}, water_plant_wait_effect = false, auto_water_plants = false, water_plant_selected_cans = {}, water_plant_mode =
...
"Farm Middle", sprinkler_place_target_plant = "", sprinkler_place_saved_position = {}, sprinkler_place_teleport = false, sprinkler_place_delay = .6, shovel_plant_variant_blacklist = {}, hide_plant_models = false, sell_when_backpack_full = false, auto_idle_touch = true, auto_collect_event_seeds = true, auto_collect_drop_seeds = true, is_frist_run = false, seed_avoid = {}, enabled_seed_shop = false, gear_shop_avoid = {}, enabled_gear_shop = false, auto_collect_fruit_enabled = false, collect_fruit_list = {}, auto_seedplace = false, allowed_seedsplace = {}, seed_place_default_target = 10, seed_place_delay = .3, seed_place_mode =
"Random", seed_place_saved_position = {}, seed_place_max_garden_plants = 800, seed_place_overrides = {}, seed_place_wall_mode = false, seed_place_stack_mode = false, seed_place_stack_mode_underground = false, auto_sell_sellallinventory = false, turbo_sell = false, hide_log_ui = false, collection_teleport = true, char_farm_middle = false, auto_shovel_fruits = false, shovel_fruit_types = {}, shovel_mutation_whitelist = {}, shovel_mutation_blacklist = {}, shovel_min_weight = 0, shovel_max_weight = 99, shovel_variants = { Normal = true, Gold = true, Rainbow = true }, auto_shovel_plants = false, shovel_plant_types = {}, shovel_plant_min_height = 0, shovel_plant_max_height = 200, shovel_plant_variants = {}, shovel_growing_plants = false, shovel_plants_keep = 10, trowel_plant_types = {}, trowel_use_fixed_spot = true, trowel_saved_position = {}, pet_return_farm = false, pet_return_farm_timer = 60, mail_auto_send_enabled = false, mail_auto_accept = false, mail_include_comment = false, mail_next_send_at = 0, mail_manual_order = {}, mail_auto_rules = {}, mail_receipts = {}, mail_ignore_batch_limit = false }
...
local s = e .. ("/" .. (tostring(J.player_userid) .. "gag2.json"))
u.Save = { RequireSave = false, SaveData = function(G)
    ...
    if not isfile(s) then return end
    local G = readfile(s)
    ...
    if G then writefile(s, V) else end
end, SaveLoop = function() task.spawn(function() while true do
            task.wait(.5)
...
u.Save.SaveLoop()
local N = { k = 1000.0, K = 1000.0, m = 1000000.0, M = 1000000.0, b = 1000000000.0, B = 1000000000.0, t = 1000000000000.0, T = 1000000000000.0, q = 1e+015, Q = 1e+015, Qa = 1e+015, qi = 1e+018, Qi = 1e+018, sx = 1e+021, Sx = 1e+021, sp = 1e+024, Sp = 1e+024, oc = 1e+027, Oc = 1e+027, no = 1e+030, No = 1e+030, dc = 1e+033, Dc = 1e+033 }
local W = { "k", "M", "B", "T", "Qa", "Qi", "Sx", "Sp", "Oc", "No", "Dc" }
...
d.SellManager = { SellFruit = function(G)
    ...
    for G, V in ipairs(G) do
        local y = V.id
        d.SellManager.SellFruit(y)
        task.wait(.05)
    end
end, SellAll = function()
    if not Y.auto_sell_sellallinventory then return end
    if Y.sell_when_backpack_full then if not d.FruitCollect.IsMaxFruitInventory() then return false end end
    if d.PlayerData.GetFruitCount() <= 0 then return end
    local G = y.Networking and (y.Networking.NPCS and y.Networking.NPCS.SellAll)
    if not G or type(G.Fire) ~= "function" then return false, nil end
    local V = G:Fire()
    return true, V
end }
d.FruitFilters = { GetFruitIds = function(G)
    ...
    return V
end, GetReadyFruits = function(G)
    local V = {}
    local y
    if type(G) == "string" and G ~= "" then y = d.FruitFilters.GetFruitsUsingName(G) else y = d.FruitFilters
        .GetAllFruits() end
...
d.ShovelFruits = { MaxPerLoop = 60, RequestDelay = .1, EquippedBySystem = false, TotalRemoved = 0, LastLoopRemoved = 0, SetStatus = function(
    ...
    string.format("Waiting | Removed: %d", d.ShovelFruits.TotalRemoved), "#CFCFCF") end, GetMutationNames = function()
    local G = {}
    local V = {}
    local Z = y.SharedModules
    local j = Z and Z:FindFirstChild("MutationData")
    if j then for y, Z in ipairs(j:GetChildren()) do if Z:IsA("ModuleScript") then
                local y = Z.Name
    ...
    table.sort(G)
    return G
end, GetVariantNames = function() return { "Normal", "Gold", "Rainbow" } end, GetFruitTypeDropdown = function()
    local G = {}
    for V, y in ipairs(J.AllSeedsDataTable) do
        if type(y) ~= "table" then continue end
        local Z = y.name
    ...
    return false
end, PassesMutationFilters = function(G)
...
d.FruitCollect = { FruitBucket = {}, FruitBucketIndex = 1, IsMaxFruitInventory = function()
    ...
    local V = 0
    if #d.FruitCollect.FruitBucket > 0 then if d.FruitCollect.IsFarFromGarden() and Y.collection_teleport then
...
d.PetFinderPremium = { Started = false, Busy = false, Folder = nil, Pets = {}, FolderConnections = {}, Pending = nil, Handled = {}, RetryAt = {}, Attempts = {}, ExpectedCounts = {}, MaxAttempts = 2, SizeRanks = { Normal = 1, Big = 2, Huge = 3 }, VariantRanks = { Normal = 1, Rainbow = 2 }, NextScanAt = 0, NextHopAt = 0, ScanDelay = 10, ConfirmTimeout = 3, SetStatus = function(
    ...
    local c = { ref = V, data = G, countBefore = d.PetFinderPremium.CountOwnedForRule(G.name, Z), startedAt = os.clock(), confirmed = false }
    ...
    if T then repeat
            ...
            task.wait(.05)
        until os.clock() - c.startedAt >= d.PetFinderPremium.ConfirmTimeout end
    local u = c.confirmed
    if u then
...
J.WebhookStatusText = ""
d.Webhooks = { SetStatus = function(G, V)
    G = tostring(G or "")
    if G == "" then
        J.WebhookStatusText = ""
        return
...
d.GardenItems.EventSeedCollectSystem = { Claim = function(G)
    ...
    local Z, j = pcall(function()
        task.spawn(function() for G = 1, 3, 1 do
                d.ProximityPrompt.ActivateProximityPrompt(V)
                task.wait()
            end end)
        if not d.Teleport.TeleportTo(G, true, y) then return false end
    ...
    task.spawn(function() while true do
            task.wait(.5)
            if Y.auto_collect_event_seeds then for G, V in ipairs(y.EventSeedDrops:GetChildren()) do if d.GardenItems.EventSeedCollectSystem.Claim(V) then
                        task.wait(.5) end end end
        end end)
end }
...
J.HomeDashboardUi = function()
    ...
    local V = j:AddTab({ Name = "Home", Description = "Game Server Version: " .. G, Icon = "house" })
...
J.MailUi = function()
    ...
    if V then
        ...
        q = V:AddDropdown("mail_manual_category_ui",
            { Values = { "Seeds", "Pets" }, Default = G, Multi = false, Text = "\240\159\147\166 Item Category", Tooltip =
            "Choose seeds or pets.", Callback = function(V)
    ...
    if y then
        ...
        H = y:AddDropdown("mail_rule_category_ui",
            { Values = { "Seeds", "Pets" }, Default = V, Multi = false, Text = "\240\159\147\166 Item Category", Tooltip =
            "Choose seeds or pets.", Callback = function(G)
...
J.PremiumUi = function()
    ...
    if y then
        ...
        local V = y:AddButton({ Text = "\226\156\133 All", Func = function()
        ...
        y:AddButton({ Text = "\240\159\147\140 Copy Current Position", Tooltip =
...
J.CollectUi = function()
    ...
    if V then
        ...
        V:AddToggle("enablefruitcollector",
            ...
            "Collects frutis and teleports you to the farm.", Callback = function(G)
...
J.SellingUi = function()
    ...
    if V then
        V:AddToggle("sell_when_backpack_full_ui",
            ...
            "Automatically sells all fruits when your backpack is full.", Callback = function(G)
        ...
        V:AddToggle("autopsellfruits",
            ...
            .auto_sell_sellallinventory, Tooltip = "Sells all your fruits.", Callback = function(G)
...
local L = d.AntiFling
local m = y.Players.LocalPlayer
local function K(G, V) if V:IsA("BasePart") then
        L._trackedParts[V] = true
        if not L._playerParts[G] then L._playerParts[G] = {} end
        L._playerParts[G][V] = true
    end end
local function b(G, V)
    L._trackedParts[V] = nil
    if L._playerParts[G] then L._playerParts[G][V] = nil end
end
local function S(G)
    if L._charConns[G] then
        for G, V in ipairs(L._charConns[G]) do V:Disconnect() end
        L._charConns[G] = nil
    end
    if L._playerParts[G] then
        for G in pairs(L._playerParts[G]) do L._trackedParts[G] = nil end
        L._playerParts[G] = nil
    end
end
local function z(G, V)
    S(G)
    L._charConns[G] = {}
    for V, y in ipairs(V:GetDescendants()) do K(G, y) end
    table.insert(L._charConns[G], V.DescendantAdded:Connect(function(V) K(G, V) end))
    table.insert(L._charConns[G], V.DescendantRemoving:Connect(function(V) b(G, V) end))
end
local function f(G)
    if G == m then return end
    if G.Character then z(G, G.Character) end
    if not L._playerConns[G] then L._playerConns[G] = G.CharacterAdded:Connect(function(V) z(G, V) end) end
end
local function t(G)
    if L._playerConns[G] then
        L._playerConns[G]:Disconnect()
        L._playerConns[G] = nil
    end
    S(G)
end
function d.AntiFling.Start()
    if L.Enabled then return end
    L.Enabled = true
    table.insert(L._mainConns, y.Players.PlayerAdded:Connect(f))
    table.insert(L._mainConns, y.Players.PlayerRemoving:Connect(t))
    table.insert(L._mainConns,
        y.RunService.PreSimulation:Connect(function() for G in pairs(L._trackedParts) do G.CanCollide = false end end))
    for G, V in ipairs(y.Players:GetPlayers()) do f(V) end
end

function d.AntiFling.Stop()
    if not L.Enabled then return end
    L.Enabled = false
    for G, V in ipairs(L._mainConns) do V:Disconnect() end
    L._mainConns = {}
    for G, V in pairs(L._playerConns) do V:Disconnect() end
    L._playerConns = {}
    for G, V in pairs(L._charConns) do for G, V in ipairs(V) do V:Disconnect() end end
    L._charConns = {}
    L._trackedParts = {}
    L._playerParts = {}
end
